...
| `POLL_INTERVAL_SECONDS` | 아니요 | `300` | 폴링 주기(초) |
...
폴링은 앱 시작 시점이 아니라 시계의 주기 경계에 맞춰 실행됩니다. 기본 5분 주기에서는 매시 `00`, `05`, `10`, `15`분 경계 10초 전에 CGV Lightpanda 탭을 준비합니다. 정각부터 `+30초`까지 5초 간격으로 최대 7회 조회하며, 각 조회에서 새 회차가 발견되면 다음 조회를 기다리지 않고 바로 알림을 전송합니다. 조회가 다음 시각까지 끝나지 않으면 같은 지점 요청을 겹치지 않도록 지난 시각은 건너뜁니다. 집중 조회가 끝나면 CGV 탭을 닫습니다. 메가박스도 같은 시각에 HTTP로 조회합니다.
