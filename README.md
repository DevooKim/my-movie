...
`공지`와 `안내`는 모든 구성원이 볼 수 있는 최상위 읽기 전용 채널입니다. `안내`의 `🔔 알림 채널 보기` 버튼을 누르면 Bot이 `영화 알림` 역할을 부여하고 알림 카테고리를 표시합니다. 안내 메시지에는 서비스 이미지가 함께 게시됩니다. Bot 역할은 `영화 알림` 역할보다 위에 있어야 합니다.
