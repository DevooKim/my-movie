...
`제어` 채널은 초기화한 사용자와 Bot만 접근할 수 있습니다. 나머지 채널은 서버 구성원이 읽고 이모지 반응을 추가할 수 있지만 메시지를 작성할 수 없는 공개 읽기 전용 채널입니다. Discord 관리자와 Bot은 메시지를 작성할 수 있습니다. `제어` 채널의 선택 메뉴에서 대상을 고른 뒤 `알림 켜기` 또는 `알림 끄기` 버튼을 누릅니다.
...
공급자 조회가 실패하면 비공개 `제어` 채널에 상세 오류를 한 번 보내고, 공개 `서버-상태` 채널에는 상세 원인을 제외한 상태 메시지를 보냅니다. 연속 실패는 중복 알리지 않으며 이후 조회가 성공하면 두 채널에 정상화 알림을 한 번 보냅니다.
